...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(a: TweetSet, b:Tweet) : Tweet ={
    val max = a.filter(x => x.retweets > b.retweets)
    if(max.isEmpty) b
    else mostRetweetedAcc(max, max.elem)
  }

  def mostRetweeted: Tweet = mostRetweetedAcc(this,this.elem)
